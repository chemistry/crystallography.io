...
export const CompoundName: FC<{ model: StructureModel; highlight?: HighlightFunction }> = ({
  model,
  highlight,
}) => {
  ...
  return <span>{compoundName ? CapitalizeFirstLetter(compoundName) : model.id || ''}</span>;
};

export const CompoundFormula: FC<{ model: StructureModel }> = ({ model }) => {
...
const getAuthorDetails = (
  ...
  if (author.link) {
    ...
    return (
      <NavLink to={authorUrl} title={author.name} key={i}>
        {authorName}
      </NavLink>
    );
  }
...
export const AuthorsList: FC<{ model: StructureModel }> = ({ model }) => {
