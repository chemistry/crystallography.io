...
export const { loadAuthorsListPageStarted, loadAuthorsListSuccess, loadAuthorsListFailed } =
  authorsListPageSlice.actions;
export const authorsListPageReducer = authorsListPageSlice.reducer;

export const fetchAuthorsListData =
  ({ page }: { page: string }): AppThunk =>
  async (dispatch) => {
    try {
