...
const searchResultsSlice = createSlice({
  ...
  reducers: {
    ...
    loadStructureListSuccess(state, { payload }) {
      ...
      // structureIds preserved from previous reducer
    },
...
export const updateSearchResults =
  (data: SearchByStructureResponse): AppThunk =>
  async (dispatch, getState) => {
    ...
    dispatch(searchResultsUpdate({ ids: structuresToLoad, meta: data.meta }));
...
export const fetchSearchResultsData =
  ({ id, page }: { id: string; page: string }): AppThunk =>
  async (dispatch) => {
    try {
      dispatch(searchResultsStart({ id, page }));
      ...
      dispatch(searchResultsSuccess({ ids: structuresToLoad, meta: data.meta }));
