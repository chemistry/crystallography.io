...
export const { searchByStructure, searchByStructureSuccess, searchByStructureFailed } =
  searchByStructureSlice.actions;
export const searchByStructureReducer = searchByStructureSlice.reducer;
...
export const searchStructureByStructure =
  ({ molecule }: { molecule: any }): AppThunk =>
  async (dispatch) => {
    try {
      ...
      dispatch(searchByStructureSuccess({ ids: structuresToLoad, meta: data.meta }));
