...
const searchByAuthorSlice = createSlice({
  ...
  reducers: {
    ...
    searchStructureByAuthorIdsSuccess(
      state,
      action: {
...
export const {
  searchStructureByAuthorStart,
  loadStructureListSuccess,
  searchStructureByAuthorIdsSuccess,
  searchStructureByAuthorFailed,
} = searchByAuthorSlice.actions;
export const searchByAuthorReducer = searchByAuthorSlice.reducer;
...
export const searchStructureByAuthor =
  ({ name, page }: { name: string; page: number }): AppThunk =>
  async (dispatch) => {
    try {
