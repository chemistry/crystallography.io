...
const searchByFormulaSlice = createSlice({
  ...
  reducers: {
    ...
    searchStructureByFormulaIdsSuccess(
      state,
      action: {
...
export const {
  searchStructureByFormulaStart,
  loadStructureListSuccess,
  searchStructureByFormulaIdsSuccess,
  searchStructureByFormulaFailed,
} = searchByFormulaSlice.actions;
export const searchByFormulaReducer = searchByFormulaSlice.reducer;
...
export const searchStructureByFormula =
  ({ formula, page }: { formula: string; page: number }): AppThunk =>
  async (dispatch) => {
    try {
