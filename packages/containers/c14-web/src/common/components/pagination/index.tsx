...
const PageLinkTemplate = ({
  ...
  return (
    <NavLink to={url + '/' + page} className="c-page-link">
      {page}
    </NavLink>
  );
};
...
export const Pagination = ({
  ...
  const pageItems = pagesNum.map((pageNum) => {
    ...
    return (
      <li className={className} key={pageNum}>
        <PageLinkTemplate url={url} page={pageNum} onPageNavigate={onPageNavigate} />
      </li>
    );
  });
  ...
  return (
    <div className="c-pagination">
      <ul className="c-pagination-list">{pageItems}</ul>
    </div>
  );
};
